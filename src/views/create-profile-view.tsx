...
export const CreateProfileView: React.FC<Props> = ({ className }) => {
	...
	return (
		<div
			className={cn(
				'bg-surface border-primary-200 w-full max-w-96 rounded-lg border shadow-md',
				className
			)}
		>
			<div className="p-6">
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					<label className="block">
						<input
							...
							autoFocus
							maxLength={50}
							className="border-border focus:ring-primary-500 focus:border-primary-500 w-full rounded-lg border px-4 py-2 focus:ring-2 focus:outline-none"
						/>
					</label>
					<button
						type="submit"
						className="bg-primary-500 hover:bg-primary-600 w-full rounded-lg px-4 py-2 font-medium text-white disabled:cursor-not-allowed disabled:opacity-50"
